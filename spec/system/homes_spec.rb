...
RSpec.describe "Homes", type: :system do
  ...
  let!(:review4) { create(:review, user_id: 2, store_id: 4, reviews_title: "楽器売却", score: 2, store_price: 9999, instrument_name: "Tuba", store_reviews: "test") }
  ...
  describe "Homes#index" do
    scenario "最新の口コミTop3が表示される" do
      expect(response.body).to have_content review.reviews_title
      expect(response.body).to have_content review2.reviews_title
      expect(response.body).to have_content review3.reviews_title
      expect(response.body).not_to have_content review4.reviews_title
    end

    scenario "評価が高い店Top3が表示される" do
      expect(response.body).to have_content store.name
      expect(response.body).to have_content store2.name
      expect(response.body).to have_content store4.name
      expect(response.body).not_to have_content store3.name
    end
